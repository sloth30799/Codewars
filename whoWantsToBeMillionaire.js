...
function getTotalCashPrize(prizeFund, correctAnswers, playerActions) {
	...
	prizeFund.forEach((x, i) => {
		if (i % 5 === 0) {
			safeHeavens.push(prizeFund.slice(0, i));
		}
	});

	safeHeavens = safeHeavens.map((x) => x.reduce((total, x) => total + x, 0));

	const actionsWithLifeLines = playerActions.map((x) => x.split("")).flat().filter(x => Number(x));
	let lifeLinesUsed = actionsWithLifeLines.length

	if (playerActions.includes("W")) {
		let prize = prizeFund
			.slice(0, playerActions.length - 1)
			.reduce((total, x) => total + x, 0);
		return [prize, lifeLinesUsed];
	} else if (playerActions[playerActions.length - 1].split('').includes("X")) {
		let safeHeavensCount = Math.floor(playerActions.length / 5);
    if (playerActions.length % 5 === 0) {
      safeHeavensCount--
    }

        return [safeHeavens[safeHeavensCount], lifeLinesUsed]
	} else if (
		playerActions.length === 15 &&
			playerActions[playerActions.length - 1].split('').includes(correctAnswers[correctAnswers.length - 1])
	) {
        prize = prizeFund.reduce((total, x) => total + x, 0);
		return [prize, lifeLinesUsed];
	} else {
		return [0, lifeLinesUsed];
	}
}
